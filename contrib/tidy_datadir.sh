...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Thronus database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Thronus datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Thronus datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Thronus 0.7 datadir."
    ;;
  3)
    echo "Detected Thronus pre-0.8 datadir."
    ;;
  4)
    echo "Detected Thronus 0.8 datadir."
    ;;
esac
